...
docvault は、雑メモをユーザーが事前分類せず `inbox/` に投げ、エージェントが軽く整えて Filed Note にするための Git-backed inbox-zero vault です。目的は文書管理アプリを作ることではなく、判断を policy として明示し、実行と履歴を標準部品へ寄せることです。
...
- capture-first: ユーザーは `inbox/` に入れるだけにする。事前のフォルダ判断を要求しない。
...
`examples/` は新しい処理 mechanism ではなく、`AGENTS.md` の判断境界を specification by example として固定するための非運用 Example Case です。実行対象は引き続き `inbox/*.md` の Capture に限定します。
